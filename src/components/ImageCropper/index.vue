...
import { b } from '@/utils/uuid'

export default {
  props: {
    // 域，触发事件会带上（如果一个页面多个图片上传控件，可以做区分
    field: {
      type: String,
      'default': 'avatar'
    },
    // File name. Default: a random v4 UUID
    fileName: {
      type: String,
      'default': b()
    },
  ...
  methods: {
    ...
    upload () {
      let that = this,
        ...
        fmData = new FormData()
      fmData.append(field, data2blob(createImgUrl, mime), fileName + '.' + imgFormat)
      // 添加其他参数
